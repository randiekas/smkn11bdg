...
class Transaksi_penerimaan_skr extends CI_Controller {
	 function __construct()
	 {
		 parent::__construct();
		 $this->load->model("penerimaan/M_Transaksi_penerimaan_skr","model");
	 }
   ...
   */
	...
	{
		$data["content"] = "penerimaan/transaksi_penerimaan_skr/index";
	...
	public function grid_data_cicilan()
	{
		...
		$result = $this->db->query("call app_penerimaan_sukarela('view','{$nis}','{$idpenerimaan}','','','','{$idtahunbuku}');")->result();
		$this->M_API->JSON($result);
	}
	...
	public function save_cicilan()
	{
		...
		$result = $this->db->query("call app_penerimaan_sukarela('insert','{$nis}','{$idpenerimaan}','{$besar}','{$keterangan}','{$petugas}','{$idtahunbuku}');")->last_row();
