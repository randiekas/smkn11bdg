...
var source =
{
  ...
  deleterow: function (ids,commit) {
      ...
      rowsSelected.forEach(function(e){
		...
		deleterow = deleterow+currItem.nis+"("+currItem.keterangan+"),";
        x++;
      });
     ...
     confirm = function(){
		$.ajax({
			 ...
			 success: function (data, status, xhr) {
				 ...
				 if(data.status=="success")
				 {
					 Notification.open("Mutasi telah berhasil dibatalkan","success");
					elementJqxGridReligion.jqxGrid('updatebounddata', 'sort');
				}
				 else{
						Notification.open(data.message,"error");
	...
	messageDelete = "Apakah anda yakin akan membatalkan mutasi ?<br> Mutasi  : "+deleterow;
