jqxGridTujuan = $("#jqxGridTujuan");
jqxGridTujuanDataFields = [
	{ "name": "replid"},
	{ "name": "nis"},
	{ "name": "nisn"},
	{ "name": "nik"},
...
var updateAction = function (rowid, rowdata, commit) {
                 ...
                 jqxGridTujuan.jqxGrid('showloadelement');
                 $.ajax({
                     dataType: 'json',
                     ...
                     success: function (data, status, xhr) {
						 ...
						 if(data.status=="success")
						 {
							 ...
							 jqxGridTujuan.jqxGrid('hideloadelement');
							 commit(true);
						 }
						 else{
                 ...
                 $.ajax({
                     ...
                     success: function (data, status, xhr) {
                         ...
                         if(data.status=="success")
						 {
							 ...
							 jqxGridTujuan.jqxGrid('updatebounddata', 'sort');
						 }
						else{
							if(data.message)
							{
...
var deleteAction= function (ids,commit) {
      ...
      var rowsSelected = jqxGridTujuan.jqxGrid('getselectedrowindexes');
      x = 0;
      rowsSelected.forEach(function(e){
        rows[x] = jqxGridTujuan.jqxGrid('getrowid',e);
		currItem = jqxGridTujuan.jqxGrid('getrowdata', e);
  ...
  var dataSource = {
	  datatype: "json",
	  datafields: jqxGridTujuanDataFields,
...
jqxGridTujuan.on('rowselect', function (event) {
  ...
  var row = event.args.row;
	...
	{
		setTimeout(function(){
			jqxGridTujuan.jqxGrid('unselectrow', event.args.rowindex);
		},200);
	}
	else{
