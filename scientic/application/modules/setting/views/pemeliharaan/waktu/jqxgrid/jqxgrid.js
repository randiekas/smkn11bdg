...
source =
{
  ...
  deleterow: function (ids,commit) {
	  ...
	  hari = <?=getDataField("referensi","ref_kode","ref_nama","ref_grup","54")?>;
	  nama_hari = "";
	  deleterow = "";
      rows = new Array();
      ...
      rowsSelected.forEach(function(e){
		...
		hari.forEach(function(val){
			if(currItem.kd_hari==val.value)
			{
				nama_hari = val.label;
			}
		})
		deleterow = deleterow+currItem.kd_program+" - "+nama_hari+" - "+currItem.sesi+" - "+currItem.sks+",";
        x++;
      });
