...
class M_jqxtree extends CI_Model{
  /**
  ...
  public function createKeyJson()
  {
    $modules = array();
		$this->db->where("na","A");
		foreach($this->db->get("module_level")->result() as $row)
		{
			...
			if($Q->num_rows()>=1)
			{
				...
				$m = $Q->last_row();
				$group_url = $this->getGroup($m->parent_id);
        $module["id"] = $m->id;

				$module["parent_id"] = $m->parent_id;
				$module["name"] =  $m->name;
				$module["url"] =  $group_url."/".$m->url;
				$module["slug"] =  $group_url."/".$m->slug;
        ...
        $module["id"] = $m->id;
        $module["parent_id"] = $m->parent_id;
				$module["url"] =  $group_url."/".$m->url."/$1";
				$module["slug"] =  $group_url."/".$m->slug."/(:any)";
        $module["module_type"] =  $m->module_type;
				$module["module"] =  FALSE;
				$modules[] = $module;
